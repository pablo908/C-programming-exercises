...
void opcionX(SortedMap *files_map, SortedMap *movies_map, movie *aux_movie){
    ...
    while(movies_map){
        ...
        removeAllSortedMap(movies_map);
        movies_map = nextSortedMap(files_map);
    }
    removeAllSortedMap(files_map);
...
void opcion3(char* lista, movie *aux_movie, SortedMap *found, SortedMap *files_map){
    ...
    if (found == NULL){
        printf("Lista no encontrada en el registro\n");
        getchar();
        return;
    }
...
int main()
{
    ...
    char *archivo = NULL;
    char lista[100];
    char *files_map_key = NULL;
    FILE *parch = NULL;
    char linea[1024];
    char field[15][1024];
    int i = 0;
    movie *aux_movie = NULL;
    SortedMap *found = NULL;
    SortedMap *files_map = createSortedMap(key_cmp);
    SortedMap *movies_map = NULL;
    //se muestra las funciones del menu de la app
    show_app_menu();

    //declaracion opcion menu app
    char opcion;

    //menu a nivel de codigo
    while(1){
        scanf("%c",&opcion);
        switch(opcion){
            case '1':
                opcion1(parch,movies_map,files_map,archivo,i,linea,field);
                /*movies_map = createSortedMap(id_cmp);
                printf("Ingrese el nombre de la lista .csv que desea ingresar\n");
                archivo = (char*)malloc(sizeof(char)*100);
                scanf("%s",archivo);
                ...
                break;
            case '2':
                opcion2(files_map_key,files_map);
                /*files_map_key = firstSortedMapKey(files_map);
                while(files_map_key){
                    printf("%s\n", files_map_key);
                    files_map_key = nextSortedMapKey(files_map);
                ...
                break;
            case '3':
                opcion3(lista,aux_movie,found,files_map);
                /*scanf("%s",lista);
                found = searchSortedMap(files_map,lista);
                aux_movie = firstSortedMap(found);
                while(aux_movie){
                ...
                break;
            case '4':;
                opcion4(files_map,movies_map,aux_movie);
                /*int escrita = 0;
                char id[20];
                scanf("%s",id);
                movies_map = firstSortedMap(files_map);
                ...
                break;
            case '5':;
                opcion5(files_map,movies_map,aux_movie);
                /*char id2[20];
                scanf("%s",id2);
                movies_map = firstSortedMap(files_map);
                while(movies_map)
            ...
            case '6':
                if(files_map){
                    opcion6(files_map,movies_map,aux_movie);
                }
                /*int maximum = -1;
                int cont = 0;
                SortedMap *maximum_map = NULL;
                ...
                break;
            default:
                opcionX(files_map,movies_map,aux_movie);
                //printf("Gracias por usar la app.\n");
                return 0;
        }
    }
